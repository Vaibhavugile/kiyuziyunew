...
if (item.isLastQty) {
  ...
  let lastStockText = "LAST STOCK SOLD";

  if (item.lastQtyVariation) {

    const color = item.lastQtyVariation.color || "";
    const size = item.lastQtyVariation.size || "";

    lastStockText =
      `LAST STOCK SOLD (${color} ${size})`;
  }

  doc
    .fontSize(8)
    .fillColor("#ff0000")
    .text(lastStockText, textX, nextY);
...
const shipping1 = orderData.billingInfo || {};

doc
  .fontSize(18)
  .fillColor("#000000")
  .text("SHIPPING ADDRESS", 50, 50, {
    align: "center",
    width: 500,
  });
...
if (shipping1.addressLine1) {
  doc.fontSize(12).text(shipping.addressLine1, 90, addressY);
  addressY += 20;
}

if (shipping1.addressLine2) {
  doc.text(shipping1.addressLine2, 90, addressY);
  addressY += 20;
}
...
doc.text(
  `${shipping1.city || ""}, ${shipping1.state || ""} - ${shipping1.pincode || ""}`,
  90,
  addressY
);
